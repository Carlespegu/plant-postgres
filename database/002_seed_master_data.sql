...
CREATE TABLE IF NOT EXISTS installations (
    ...
    installation_type_id    SMALLINT REFERENCES installation_types(id),
...
CREATE INDEX IF NOT EXISTS idx_installations_type_id
    ON installations(installation_type_id);
...
-- =========================================================
-- 7) PLANT SPECIES (CATÀLEG MESTRE)
-- =========================================================
...
CREATE TABLE IF NOT EXISTS plant_species (
    ...
    plant_type_id               SMALLINT REFERENCES plant_types(id),
    default_pot_type_id         SMALLINT REFERENCES pot_types(id),
    size_category               VARCHAR(50),
    sunlight_level              VARCHAR(50),
    watering_level              VARCHAR(50),
    humidity_level              VARCHAR(50),
    ideal_temp_min_c            NUMERIC(5,2),
    ideal_temp_max_c            NUMERIC(5,2),
    ideal_soil_moisture_min     NUMERIC(5,2),
    ideal_soil_moisture_max     NUMERIC(5,2),
    ideal_light_min             INTEGER,
    ideal_light_max             INTEGER,
    notes                       TEXT,
    created_at                  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at                  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT chk_plant_species_temp_range
        CHECK (
            ideal_temp_min_c IS NULL
            OR ideal_temp_max_c IS NULL
            OR ideal_temp_min_c <= ideal_temp_max_c
        ),
    CONSTRAINT chk_plant_species_soil_range
        CHECK (
            ideal_soil_moisture_min IS NULL
            OR ideal_soil_moisture_max IS NULL
            OR ideal_soil_moisture_min <= ideal_soil_moisture_max
        ),
    CONSTRAINT chk_plant_species_light_range
        CHECK (
            ideal_light_min IS NULL
            OR ideal_light_max IS NULL
            OR ideal_light_min <= ideal_light_max
        )
);

CREATE INDEX IF NOT EXISTS idx_plant_species_plant_type_id
    ON plant_species(plant_type_id);

CREATE INDEX IF NOT EXISTS idx_plant_species_default_pot_type_id
    ON plant_species(default_pot_type_id);

-- =========================================================
-- 8) PLANTS (PLANTA CONCRETA DE L'USUARI)
-- =========================================================
...
CREATE TABLE IF NOT EXISTS plants (
    ...
    pot_type_id                 SMALLINT REFERENCES pot_types(id),
    planted_at                  TIMESTAMPTZ,
    acquired_at                 TIMESTAMPTZ,
    custom_temp_min_c           NUMERIC(5,2),
    custom_temp_max_c           NUMERIC(5,2),
    custom_soil_moisture_min    NUMERIC(5,2),
    custom_soil_moisture_max    NUMERIC(5,2),
    custom_light_min            INTEGER,
    custom_light_max            INTEGER,
    status                      VARCHAR(50),
    notes                       TEXT,
    created_at                  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at                  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT chk_plants_location
        CHECK (installation_id IS NOT NULL OR installation_group_id IS NOT NULL),
    CONSTRAINT chk_plants_temp_range
        CHECK (
            custom_temp_min_c IS NULL
            OR custom_temp_max_c IS NULL
            OR custom_temp_min_c <= custom_temp_max_c
        ),
    CONSTRAINT chk_plants_soil_range
        CHECK (
            custom_soil_moisture_min IS NULL
            OR custom_soil_moisture_max IS NULL
            OR custom_soil_moisture_min <= custom_soil_moisture_max
        ),
    CONSTRAINT chk_plants_light_range
        CHECK (
            custom_light_min IS NULL
            OR custom_light_max IS NULL
            OR custom_light_min <= custom_light_max
        )
);

CREATE INDEX IF NOT EXISTS idx_plants_installation_id
    ON plants(installation_id);

CREATE INDEX IF NOT EXISTS idx_plants_installation_group_id
    ON plants(installation_group_id);

CREATE INDEX IF NOT EXISTS idx_plants_species_id
    ON plants(plant_species_id);

CREATE INDEX IF NOT EXISTS idx_plants_pot_type_id
    ON plants(pot_type_id);
...
INSERT INTO installation_types (code, name, description)
VALUES
    ('monitoring_only', 'Monitoring Only', 'Installation only collects data'),
    ('plant_care', 'Plant Care', 'Installation focused on plant maintenance'),
    ('experimental', 'Experimental', 'Experimental installation'),
    ('irrigation_control', 'Irrigation Control', 'Installation with irrigation control')
...
INSERT INTO plant_types (code, name, description)
VALUES
    ('indoor', 'Indoor', 'Indoor plant'),
    ('outdoor', 'Outdoor', 'Outdoor plant'),
    ('aromatic', 'Aromatic', 'Aromatic plant'),
    ('horticultural', 'Horticultural', 'Vegetable or edible plant'),
    ('ornamental', 'Ornamental', 'Decorative plant'),
    ('succulent', 'Succulent', 'Succulent plant'),
    ('cactus', 'Cactus', 'Cactus plant')
ON CONFLICT (code) DO NOTHING;

INSERT INTO pot_types (code, name, description)
VALUES
    ('terracotta', 'Terracotta', 'Terracotta pot'),
    ('plastic', 'Plastic', 'Plastic pot'),
    ('ceramic', 'Ceramic', 'Ceramic pot'),
    ('self_watering', 'Self Watering', 'Self watering pot'),
    ('grow_bag', 'Grow Bag', 'Fabric grow bag')
ON CONFLICT (code) DO NOTHING;
